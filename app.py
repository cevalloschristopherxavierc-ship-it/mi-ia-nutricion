...
with st.sidebar:
    ...
    u_act = st.selectbox("Actividad:", ["Fútbol 2h", "Hipertrofia"])
        ...
        if foto and st.button("🔍 ANALIZAR"):
            with st.spinner("🤖 Analizando..."):
                ...
                try:
                    ...
                    d = raw.replace('g','').replace('kcal','').split('|')
                    res_c = {"n":d[0], "k":float(d[1]), "p":float(d[2]), "c":float(d[3]), "g":float(d[4])}
                except: st.error("❌ Error en IA. Intenta de nuevo.")
    ...
    if res_c:
        ...
        <div class="pizarra">
            ...
            <div style="display:flex; justify-content:space-between; margin-top:10px;">
                <div class="m-box">🔥<br><span class="verde">{res_c['k']}</span><br>Kcal</div>
                <div class="m-box">🍗<br><span class="verde">{res_c['p']}g</span><br>Prot</div>
                <div class="m-box">🍚<br><span class="verde">{res_c['c']}g</span><br>Carb</div>
                <div class="m-box">🥑<br><span class="verde">{res_c['g']}g</span><br>Gras</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        st.rerun()

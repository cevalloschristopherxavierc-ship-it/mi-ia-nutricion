...
# --- 1. CONFIGURACIÓN Y ESTILO (XAVIER 63KG) ---
...
# --- 2. PERFIL Y METAS (DETALLES BLINDADOS) ---
...
# --- 3. SIDEBAR (AGUA, PASOS Y CREADOR) ---
with st.sidebar:
...
with tabs[0]:
    ...
    with c1:
        ...
        if up and st.button("🔍 ANALIZAR"):
            with st.spinner("🤖 Jarvis forzando ruta v1beta..."):
                try:
                    img_b64 = base64.b64encode(up.read()).decode()
                    key = st.secrets["GEMINI_API_KEY"]
                    # CAMBIO CLAVE: USAMOS v1beta QUE ES DONDE ESTÁ EL FLASH ACTUALMENTE
                    ...
                    if r.status_code == 200:
                        ...
                        st.error(f"❌ Error {r.status_code}. Google dice: {r.json()['error']['message'][:50]}")
    ...
    with c2:
        ...
        with st.form("f_man", clear_on_submit=True):
            ...
            in_p = st.number_input("Prot", 0.0)
            if st.form_submit_button("💾 GUARDAR"):
                if in_n:
                    supabase.table('registros_comida').insert({"usuario":st.session_state.u_nom,"comida":in_n,"kcal":in_k,"proteina":in_p,"semana":datetime.now().strftime('%Y-%m-%d')}).execute()
                    st.rerun()

with tabs[1]:
    if k_act > 0:
        fig = go.Figure(go.Scatterpolar(r=[p_act/meta_p, k_act/meta_k, (k_act*0.5/4)/(meta_k*0.5/4), (k_act*0.25/9)/(meta_k*0.25/9)],

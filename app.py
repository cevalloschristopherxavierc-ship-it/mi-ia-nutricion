...
def init_connection():
    try:
        ...
        st.error("🚨 Error de enlace con Supabase.")
        st.stop()
...
m2.metric("🍗 Prot", f"{p_act:.1f}g/{meta_p:.1f}g")
m3.metric("🥑 Grasa", f"{g_act:.1f}g/{meta_g:.1f}g")
m4.metric("🍚 Carb", f"{c_act:.0f}g/{meta_c:.0f}g")
...
with tabs[0]:
    ...
    with r1:
        ...
        up = st.file_uploader("Captura", type=["jpg","png","jpeg"])
    ...
    with r2:
        ...
        with st.form("m_form"):
            f_n = st.text_input("Comida")
            f_k = st.number_input("Kcal", 0.0)
            f_p = st.number_input("Prot", 0.0)
...
with tabs[2]:
    st.table(df_hoy[['comida', 'kcal', 'proteina']]) if not df_hoy.empty else st.info("Sin registros.")

with tabs[3]:
    if st.session_state.get('creador', False):
        if not df_all.empty:
            target = st.selectbox("Sujeto:", df_all['usuario'].unique())
            st.dataframe(df_all[df_all['usuario'] == target], use_container_width=True)

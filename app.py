...
with t2:
        ...
        if foto and st.button("🔍 ESCANEAR CON IA"):
            with st.spinner("🤖 Jarvis analizando ración..."):
                try:
                    ...
                    if 'error' in r:
                        st.error(f"❌ Error API Gemini: {r['error']['message']}")
                    else:
    ...
    with col_m:
        ...
        with st.form("manual_entry_f"):
            ...
            k_m = col_a.number_input("Kcal:", min_value=0.0, value=500.0)
            p_m = col_b.number_input("Proteína (g):", min_value=0.0, value=30.0)
            c_m = col_a.number_input("Carbos (g):", min_value=0.0, value=50.0)
            g_m = col_b.number_input("Grasas (g):", min_value=0.0, value=15.0)
    ...
    if res_c:
        ...
        try:
            ...
            st.warning(f"⚠️ Guardado en sesión, pero no en DB. Error: {db_e}")
